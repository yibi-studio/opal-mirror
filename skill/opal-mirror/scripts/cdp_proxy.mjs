...
async function discoverChromePort() {
  ...
  for (const p of possiblePaths) {
    try {
      ...
      if (port > 0 && port < 65536) {
        ...
        if (ok) {
          const wsUrl = await fetchBrowserWebSocketUrl(port);
          if (wsUrl) {
            console.log(`[CDP Proxy] 从 /json/version 发现 Chrome WebSocket: ${port}`);
            return { port, wsPath: null, wsUrl };
          }
  ...
  for (const port of commonPorts) {
    ...
    if (ok) {
      ...
      return { port, wsPath: null, wsUrl: null };
    }
  }
...
async function connect() {
  ...
  if (!chromePort) {
    ...
    chromeWsUrl = discovered.wsUrl;
  }

  const wsUrl = chromeWsUrl || getWebSocketUrl(chromePort, chromeWsPath);
